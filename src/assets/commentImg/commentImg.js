			...
			function setViewImg(viewSrc){
	            ...
	            img.onload = function () {
	                ...
	                imgViewBox.show(0,function(){
	                	var wrapWidth = $(this).parents('.media-list').width() - 60;
	                    var scale = wrapWidth/imageWidth;
	                    if(imageWidth<wrapWidth){
	                    	$(this).css({ "width": imageWidth, "height": imageHeight}).find("img").attr('src', src);
	                		$(this).find("img").css({ "width": imageWidth, "height": imageHeight});
	                    }else{
	                    	$(this).css({ "width": imageWidth*scale, "height": imageHeight*scale }).find("img").attr('src', src);
	                		$(this).find("img").css({ "width": imageWidth*scale, "height": imageHeight*scale });
	                    }	
	                });					
	            }	            
			}
